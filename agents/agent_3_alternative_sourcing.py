...
PRODUCT_ALIASES: dict[str, str] = {
    ...
    "plastic bags": "petrochemicals",
    "plastic products": "petrochemicals",
    "palm oil": "palm oil",
    "jet fuel": "jet fuel",
    "fuel": "jet fuel",
    "electricity": "energy",
...
DEFAULT_SUPPLIERS = [
    ...
    SupplierOption(
        ...
        latitude=1.1301,
        longitude=104.052,
        products=["seafood", "poultry", "egg", "mixed food supply"],
    ...
    SupplierOption(
        ...
        latitude=10.0452,
        longitude=105.7469,
        products=["seafood", "rice", "fruit", "mixed food supply"],
    ...
    SupplierOption(
        ...
        latitude=-20.9176,
        longitude=142.7028,
        products=["vegetable", "fruit", "poultry", "mixed food supply"],
    ...
    SupplierOption(
        supplier_id="SUP-007",
        supplier_name="SK Innovation",
        country="South Korea",
        latitude=36.35,
        longitude=127.3845,
        products=["jet fuel", "energy", "petrochemicals"],
        average_cost_index=1.11,
        reliability_score=0.92,
        active_risk_tags=[],
    ),
    SupplierOption(
        supplier_id="SUP-008",
        supplier_name="Woodside Energy",
        country="Australia",
        latitude=-31.9505,
        longitude=115.8605,
        products=["jet fuel", "energy", "palm oil", "petrochemicals"],
        average_cost_index=1.14,
        reliability_score=0.94,
        active_risk_tags=[],
    ),
]
...
class TinyFishAlternativeSourcingClient:
    ...
    def __post_init__(self) -> None:
        ...
        recommendation_timeout_seconds = self._read_recommendation_timeout_seconds()
    ...
    def _build_goal(self, issue: IssueRecord) -> str:
        ...
        return (
            ...
            '{"recommendations":[{"strategy":"first_cheapest","product":"","supplier_name":"","country":"","products":[],"average_cost_index":1.0,"reliability_score":0.0,"active_risk_tags":[],"rationale":"","estimated_cost_delta_pct":0.0,"security_score":0.0,"source_label":"","source_url":""}]}. '
            f"Recommend exactly {self.max_recommendations_per_issue} best alternative sourcing option"
            f"{'' if self.max_recommendations_per_issue == 1 else 's'} outside the affected countries and not exposed to the same disruption. "
            "Prefer nearby Asia-Pacific sourcing options when realistic. "
            "Use strategy only from: first_cheapest, first_secure. "
            "If you cite a supplier analysis page, report, or article for the recommendation, include it in source_label and source_url. "
    ...
    def _normalize_result(
        ...
        for index, item in enumerate(raw_recommendations[: self.max_recommendations_per_issue], start=1):
            ...
            if not product:
                if issue.extraction.affected_products:
                    product = issue.extraction.affected_products[0]
                else:
                    product = "mixed food supply"
            ...
            if not supplier_name:
                supplier_name = f"{country} Alternative Supply Network"
            ...
            source_label = str(item.get("source_label", "")).strip() or "TinyFish recommendation source"
            source_url = self._clean_url(str(item.get("source_url", "")).strip()) or issue.source_url
            ...
            normalized.append(
                Recommendation(
                    ...
                    recommended_supplier=SupplierOption(
                        supplier_id="TF-%s-%03d" % (issue.issue_id, index),
    ...
    def _clean_url(self, value: str) -> str | None:
        if value.startswith("http://") or value.startswith("https://"):
            return value
        return None

    def _read_recommendation_timeout_seconds(self) -> int:
        env_value = os.getenv("TINYFISH_RECOMMENDATION_TIMEOUT_SECONDS")
...
class AlternativeSourcingAgent:
    ...
    def __init__(
        ...
        self.live_client = live_client or TinyFishAlternativeSourcingClient(
            max_recommendations_per_issue=max_recommendations_per_issue
        )
    ...
    def run(self, batch_id: str, issue_batch: IssueBatch) -> RecommendationBatch:
        ...
        for issue in issue_batch.issues:
            ...
            if live_recommendations:
                recommendations.extend(live_recommendations[: self.max_recommendations_per_issue])
                live_recommendation_count += min(len(live_recommendations), self.max_recommendations_per_issue)
                continue
            ...
            trimmed_fallback = issue_recommendations[: self.max_recommendations_per_issue]
            recommendations.extend(trimmed_fallback)
            fallback_recommendation_count += len(trimmed_fallback)
    ...
    def _resolve_requested_products(self, issue: IssueRecord) -> list[str]:
        ...
        keyword_products: list[str] = []
        for keyword in issue.extraction.keywords:
            normalized_keyword = self._normalize_product(keyword)
            if normalized_keyword != "mixed food supply":
                keyword_products.append(normalized_keyword)
